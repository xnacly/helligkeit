//! taken from https://github.com/torvalds/linux/blob/master/include/uapi/linux/i2c-dev.h &
//! https://github.com/torvalds/linux/blob/704340f1cd0dcef829eb62f5b48ae95a2ce17bdf/include/uapi/linux/i2c.h
//! general i2c device interface guide at:
...
impl Bus {
    pub fn open(path: impl AsRef<Path>) -> io::Result<Self> {
        Ok(Self {
            fd: OpenOptions::new().read(true).write(true).open(path)?,
        })
    }

    pub fn write(&self, address: u16, data: &[u8]) -> io::Result<()> {
        todo!()
    }

    pub fn read(&self, address: u16, data: &mut [u8]) -> io::Result<()> {
        todo!()
    }

    pub fn write_read(&self, address: u16, write: &[u8], read: &mut [u8]) -> io::Result<()> {
        todo!()
    }
}
